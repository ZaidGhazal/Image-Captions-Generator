"""This module contains the main app code."""
import os
import signal
...
def run_app(disable_training=False):
    ...
    disable_training : bool
        Whether to disable the training tab or not.
    """
    st.set_page_config(
        layout="wide",
        menu_items={
    ...
    tab1, tab2, tab3 = st.tabs(
        [
            "**\U0001F3E0 Home**",
            "**\U0001F4DD Generate Captions**",
            "**\U0001F9EE Train Model**",
        ]
    )
    ...
    with tab1:
        ...
        st.write("----------")
        st.markdown(
            """<p style="font-size: 33px; font-weight: bold;">
    ...
    with tab2:
        ...
        imgs_path = st.file_uploader(
            "Upload an image", type=allowed_img_types, accept_multiple_files=True
        )
        if st.button("Load Sample Images"):
            # load images form local directory: trial_images
            imgs_path = [
                Path("./trial_images").joinpath(img)
                for img in os.listdir("./trial_images")
                if img.split(".")[-1] in allowed_img_types
            ]
        ...
        if imgs_path != []:
            with st.spinner("Wait for it..."):
                ...
                captions = [
                    caption.strip()[:-1].strip() for _, caption in imgs_captions_list
                ]
                ...
                st.write("-------------")
                st.markdown(
                    "<p style='text-align: center; font-size: 30px; font-weight: bold;'><u>Results</u></p>",
                    unsafe_allow_html=True,
                )
                st.download_button(
                ...
                with col2:
                    ...
                    for image, caption in imgs_captions_list:
                        ...
                        st.markdown(
                            "<p style='text-align: center; font-family: Gill Sans, serif; font-size: 18px'>{}</p>".format(
                                caption.strip()[:-1].strip().capitalize()
                            ),
                            unsafe_allow_html=True,
                        )
                        st.write("---------------")
    ...
    with tab3:
        ...
        if disable_training:
            st.warning(
                """Training is disabled in the deployed app. To enable training, run the app locally.
        ...
        with col2:
            ...
            vocab_threshold = st.number_input(
                "**Select a Vocab Threshold**", 3, 15, 8, disabled=disable_training
            )
            st.write("Vocab Threshold:", vocab_threshold)

            st.write("-----------")
            num_epochs = st.number_input(
                "**Select Epochs Number**", 1, 20, 1, disabled=disable_training
            )
        ...
        if st.session_state.get("train_process") is not None:
            ...
            with col2:
                if st.button("Check Status"):
                    ...
                    if (
                        status_value == 200
                        and st.session_state.get("train_process").is_alive()
                    ):
                        ...
                        st.error(
                            "An Error occurred. Please check the Terminal/Console for more info"
                        )
